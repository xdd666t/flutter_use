self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={o2:function o2(d,e,f,g,h,i){var _=this
_.ax=0
_.e4$=d
_.fl$=e
_.eG$=f
_.eH$=g
_.ei$=h
_.eI$=i},
aRg(){var x=null,w=$.aL(),v=y.h
v=new B.o2(A.b([],y.y),A.cM(x,x,x,y.q,y.o),new A.bJ(v),new A.bJ(v),!1,!1)
v.eP()
return new B.qK(K.ig(w,v,y.a),x)},
qK:function qK(d,e){this.c=d
this.a=e},
a9r:function a9r(d){this.a=d},
a9s:function a9s(){}},C,A,D,E,F,G,H,I,K
B=a.updateHolder(c[16],B)
C=c[65]
A=c[0]
D=c[64]
E=c[73]
F=c[68]
G=c[2]
H=c[70]
I=c[61]
K=c[63]
B.o2.prototype={
aip(){++this.ax
this.cY(0)}}
B.qK.prototype={
v(d){var x=null,w=C.cD(A.ar("\u8de8\u9875\u9762-One",x,x,x,x,x,x)),v=D.dI(E.ok,!1,new B.a9r(this))
return F.cx(w,G.i,H.ci(I.hc(new B.a9s(),x,x,x,y.a),x,x),x,v)}}
var z=a.updateTypes(["ay(o2)"])
B.a9r.prototype={
$0(){var x=y.w
A.dq($.aL(),"/jumpOne/jumpTwo",A.aO(["msg","\u6211\u662f\u4e0a\u4e2a\u9875\u9762\u4f20\u9012\u8fc7\u6765\u7684\u6570\u636e"],x,x),y.b)
return null},
$S:0}
B.a9s.prototype={
$1(d){var x=null
return A.ar("\u8de8\u9875\u9762-Two\u70b9\u51fb\u4e86 "+d.ax+" \u6b21",x,x,x,A.b6(x,x,x,x,x,x,x,x,x,x,x,30,x,x,x,x,x,!0,x,x,x,x,x,x,x,x),x,x)},
$S:z+0};(function inheritance(){var x=a.inherit
x(B.o2,A.hD)
x(B.qK,A.M)
x(B.a9r,A.by)
x(B.a9s,A.b5)})()
A.bh(b.typeUniverse,JSON.parse('{"o2":{"df":[],"a7":[]},"qK":{"M":[],"c":[]}}'))
var y={a:A.m("o2"),h:A.m("bJ<~>"),y:A.m("l<~()?>"),o:A.m("K<~()>"),w:A.m("j"),b:A.m("@"),q:A.m("B?")}}
$__dart_deferred_initializers__["knkiiOgRT4eax1K/72O4K1tZcns="] = $__dart_deferred_initializers__.current
