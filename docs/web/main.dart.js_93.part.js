self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={zP:function zP(d,e,f,g,h,i,j){var _=this
_.ax=d
_.dZ$=e
_.ft$=f
_.eM$=g
_.eN$=h
_.em$=i
_.eO$=j},a2H:function a2H(){},ui:function ui(d){this.a=d},Vf:function Vf(d,e,f){var _=this
_.d=$
_.cp$=d
_.aP$=e
_.a=null
_.b=f
_.c=null},aw5:function aw5(d){this.a=d},Je:function Je(){},a2J:function a2J(){this.a=$},
aSz(){var x,w=null,v=$.aN(),u=new B.a2J()
u.a=A.b([new E.dY("Custom Animation",A.b([new E.aP("AnimationWidget","animationWidget",w,w,!1,w)],y.f))],y.a)
x=y.h
x=new B.zP(u,A.b([],y.y),A.cK(w,w,w,y.q,y.o),new A.bQ(x),new A.bQ(x),!1,!1)
x.eT()
u=y.u
x=L.iw(v,x,u)
v=$.bW
return new B.q5(x,(v==null?$.bW=C.a8:v).eP(0,w,u).ax,w)},
q5:function q5(d,e,f){this.c=d
this.d=e
this.a=f},
a2I:function a2I(d){this.a=d}},A,F,C,G,H,D,I,K,E,L
B=a.updateHolder(c[30],B)
A=c[0]
F=c[36]
C=c[2]
G=c[72]
H=c[67]
D=c[89]
I=c[70]
K=c[37]
E=c[44]
L=c[65]
B.zP.prototype={
Ay(d){var x=0,w=A.E(y.b),v=this
var $async$Ay=A.F(function(e,f){if(e===1)return A.B(f,w)
while(true)switch(x){case 0:switch(d){case"animationWidget":v.a0F()
break}return A.C(null,w)}})
return A.D($async$Ay,w)},
a0F(){var x=null
F.d5(x,x,x,x,x,x,new B.a2H(),x,x,x,x,x,x,x,x,y.b)}}
B.ui.prototype={
Y(){return new B.Vf(null,null,C.j)}}
B.Vf.prototype={
a9(){var x=A.bC(null,A.bV(0,0,2),null,null,this)
this.d=x
x.aW(0)
this.ao()},
l(){var x=this.d
x===$&&A.a()
x.l()
this.a_q()},
t(d){var x=null,w=this.d
w===$&&A.a()
w=A.c3(C.r,w,x)
return G.cJ(x,A.S(x,A.Rt(new A.dE(0,0),A.bP(A.b([A.S(x,x,C.e,C.b8,x,x,50,x,x,x,100),A.S(x,x,C.e,C.bj,x,x,50,x,x,x,100)],y.l),C.o,C.p,C.u),w),C.e,C.i,x,x,100,x,x,x,100),C.z,!1,x,x,x,x,x,x,x,x,x,x,x,x,new B.aw5(this),x,x,x,x,x,x)}}
B.Je.prototype={
l(){var x=this,w=x.aP$
if(w!=null)w.I(0,x.gfN())
x.aP$=null
x.ak()},
b4(){this.bU()
this.bJ()
this.fO()}}
B.a2J.prototype={}
B.q5.prototype={
t(d){var x=H.cE(D.SW),w=this.d.a
w===$&&A.a()
return I.cz(x,C.i,new K.Fk(w,new B.a2I(this),null),null,null)}}
var z=a.updateTypes(["ui(p)"])
B.a2H.prototype={
$1(d){return new B.ui(null)},
$S:z+0}
B.aw5.prototype={
$0(){var x=this.a.d
x===$&&A.a()
return x.iU(0,0)},
$S:0}
B.a2I.prototype={
$1(d){return this.a.c.Ay(d)},
$S:31};(function aliases(){var x=B.Je.prototype
x.a_q=x.l})();(function inheritance(){var x=a.mixinHard,w=a.inherit,v=a.inheritMany
w(B.zP,A.hL)
v(A.ba,[B.a2H,B.a2I])
w(B.ui,A.R)
w(B.Je,A.X)
w(B.Vf,B.Je)
w(B.aw5,A.bD)
w(B.a2J,A.x)
w(B.q5,A.w)
x(B.Je,A.fU)})()
A.bj(b.typeUniverse,JSON.parse('{"zP":{"dl":[],"a8":[]},"ui":{"R":[],"c":[]},"Vf":{"X":["ui"]},"q5":{"w":[],"c":[]}}'))
var y={u:A.n("zP"),h:A.n("bQ<~>"),f:A.n("l<aP>"),a:A.n("l<dY>"),l:A.n("l<c>"),y:A.n("l<~()?>"),o:A.n("L<~()>"),b:A.n("@"),q:A.n("x?")};(function constants(){D.SW=new A.aA("AnimationDemo",null,null,null,null,null,null,null,null)})()}
$__dart_deferred_initializers__["cT5TsL6DxN3xK6lRym2bYnBJm78="] = $__dart_deferred_initializers__.current
