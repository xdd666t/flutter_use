((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_27",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,D,A={
hi(d,e){return new A.OX(e,C.nF,d,null)},
OX:function OX(d,e,f,g){var _=this
_.f=d
_.r=e
_.b=f
_.a=g}}
B=c[0]
C=c[2]
D=c[58]
A=a.updateHolder(c[49],A)
A.OX.prototype={}
var z=a.updateTypes([]);(function inheritance(){var y=a.inherit
y(A.OX,D.CG)})()
B.bT(b.typeUniverse,JSON.parse('{"OX":{"er":["hj"],"b3":[],"c":[],"er.T":"hj"}}'))};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_27",e:"endPart",h:b})})($__dart_deferred_initializers__,"q7LYUQKPDvT5MwYHOyEVQYrVosE=");